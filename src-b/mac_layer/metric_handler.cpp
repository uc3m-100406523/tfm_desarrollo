...
void metric_handler::assign_metric()
{
    ...
    if(metric_t == METRIC_M_W_DELAY) metric_f_ptr = &metric_handler::m_w_delay;
}
...
float metric_handler::m_w_delay(metric_info metric_i, float current_t, int f)
{
    if(metric_i.avrg_tp!=0)
        return -(log(metric_i.delta)/metric_i.delay_t) *priority* metric_i.current_delay*metric_i.current_tp/pow(metric_i.avrg_tp, beta);// + (1 - beta)*metric_i.current_tp));
    else return -(log(metric_i.delta)/metric_i.delay_t) *priority* metric_i.current_delay*metric_i.current_tp;
}
