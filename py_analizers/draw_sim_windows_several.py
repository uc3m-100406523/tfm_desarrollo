...
def main():
    ...
    # Iterate along simulations. All "argv[" were subtituted by "argv[(13*i)+"
    n_sim = int((len(sys.argv)-1)/13)
    print(n_sim)
    for i in range(n_sim):
        ...
        # Script directory
        script_dir_ue = os.path.dirname(os.path.realpath(sys.argv[(13*i)+0]))
        ...
        if(len(sys.argv)>3): # If data is not indicated as a parameter, default values are used
            # Period
            period=float(sys.argv[(13*i)+3])
            # Logging rate (UE)
            ue_log_freq=int(sys.argv[(13*i)+4])
            # Logging rate (MAC)
            mac_log_freq=int(sys.argv[(13*i)+5])
            # MIMO layers
            mimo_layers=int(sys.argv[(13*i)+6])
            # Frequency
            frequency=float(sys.argv[(13*i)+7])
            # Bandwidth
            bandwidth=float(sys.argv[(13*i)+8])
            # UE speed
            ue_speed=float(sys.argv[(13*i)+9])
            # Max distance
            max_distance=float(sys.argv[(13*i)+10])
            # Shannon parameters
            bw_efficiency=float(sys.argv[(13*i)+11])
            sinr_offset_db=float(sys.argv[(13*i)+12])
            ul_divisor=float(sys.argv[(13*i)+13])
        else:
            # Period
            period=1
            # Logging rate (UE)
