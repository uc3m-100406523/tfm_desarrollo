...
def main():
    ...
    if(len(sys.argv)>2): # If input directory is not indicated, it is supposed to be the last execution
        logs_dir = sys.argv[2]

        ue_dir_ue = os.path.join(logs_dir, 'ue')
        log_files = sorted(glob(os.path.join(ue_dir_ue, 'ue_log_*.txt')))
    else:
        ue_dir_ue, log_files = find_ue_logs(logs_base_ue)
    ...
    if(len(sys.argv)>2): # If input directory is not indicated, it is supposed to be the last execution
        ...
        latest_log = latest_log_dir(logs_base_mac)
    ...
    if(len(sys.argv)>1): # If output directory is not indicated, it is supposed to be in emulator directory
        ...
        out_dir = os.path.join(script_dir_ue, "..", "results", log_folder_ue, "ue")
    ...
    if(len(sys.argv)>1): # If output directory is not indicated, it is supposed to be in emulator directory
        ...
        results_base = os.path.join(script_dir_mac, "..", "results", log_folder_mac, "mac")
